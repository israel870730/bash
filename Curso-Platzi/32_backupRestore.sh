...
restaurar_respaldo () {
    ...
    echo -e "\n"
    read -p "Ingrese el nombre de la BD destino: " bddDestino
    #verificar si la BDD existe
    verificarBdd=$(sudo -u postgres psql -lqt | cut -d \| -f 1 | grep -wp $bddDestino)
    if [ $? -eq 0 ]; then
        echo "Restaurando en la BDD destino: $bddDestino"
    else
        sudo -u postgres psql -c "crate database $bddDestino"
    fi
